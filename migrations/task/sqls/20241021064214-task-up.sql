...
INSERT INTO "COACH"(user_id, experience_years)
  VALUES ((
      ...
      FROM
        "USER"
      WHERE
        email = 'lee2000@hexschooltest.io'), 2);
...
INSERT INTO "COACH"(user_id, experience_years)
  VALUES ((
      ...
      FROM
        "USER"
      WHERE
        email = 'muscle@hexschooltest.io'), 2);
...
INSERT INTO "COACH"(user_id, experience_years)
  VALUES ((
      ...
      FROM
        "USER"
      WHERE
        email = 'starplatinum@hexschooltest.io'), 2);

-- 3-2. 新增：承1，為三名教練新增專長資料至 `COACH_LINK_SKILL` ，資料需求如下：
-- 1. 所有教練都有 `重訓` 專長
INSERT INTO "COACH_LINK_SKILL"(skill_id, coach_id)
SELECT
  (
    SELECT
      id
  ...
  VALUES ((
      ...
      WHERE
        ...
        FROM
          "SKILL"
        WHERE
          name = '重訓'), '重訓基礎課', '2024-11-25 14:00:00', '2024-11-25 16:00:00', 10, 'https://test-meeting.test.io');
...
(
  ...
  WHERE
    ...
    OR email = 'richman@hexschooltest.io';
...
INSERT INTO "COURSE_BOOKING"(user_id, course_id, booking_at, status)
  VALUES ((
      ...
      WHERE
        ...
        WHERE
          u.name = '李燕容'), '2024-11-24 17:10:25', '即將授課');
    ...
    WHERE
      ...
      AND user_id =(
        ...
        WHERE
          name = '王小明')
      GROUP BY
        user_id) AS cb ON cp.user_id = cb.user_id;
...
SELECT
  ...
  s.name
FROM
  "COACH_LINK_SKILL" AS cls
  INNER JOIN "COACH" AS c ON cls.coach_id = c.id
  INNER JOIN "SKILL" AS s ON cls.skill_id = s.id
...
SELECT
  s.name,
  count(cls.skill_id) AS coach_total
FROM
  "COACH_LINK_SKILL" AS cls
  INNER JOIN "SKILL" AS s ON cls.skill_id = s.id
GROUP BY
...
SELECT
  c_package."name" AS 組合包方案名稱,
  count(*) AS 銷售數量
FROM
  "CREDIT_PURCHASE" c_purchase
  INNER JOIN "CREDIT_PACKAGE" c_package ON c_purchase.credit_package_id = c_package.id
WHERE
  c_purchase.created_at >= '2024-11-01 00:00:00'
  AND c_purchase.created_at <= '2024-11-30 23:59:59'
GROUP BY
  c_package.id;
